...
process *crear_Proceso(int _id, int _delay, char *_nombretarea, char *_nombreProceso,int _estado,FN_ACCION _proceso_realizar){
        
	process *_p = (process *)malloc(sizeof(process));
	_p->id = _id;
	_p->delay = _delay;
	strcpy(_p->nombrePROCESO, _nombreProceso);
	strcpy(_p->nombretarea, _nombretarea);
	_p->estado = _estado;
...
void ejecutar_Procesos(array_procesos *_array){
	printf("-----> Procesos = [%d]\n", cont_procesos);
	pthread_t tareas[cont_procesos];
	for(int i = 0; i < cont_procesos; i++){
		if(_array[i].proceso.estado == ACTIVO){
			...
			printf("<=================================================================================>\n");
			pthread_create(&tareas[i], NULL, _array[i].proceso.proceso_realizar, (void *)_array[i].proceso.nombrePROCESO);
